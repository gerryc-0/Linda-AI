...
for i in range(num_patients):
    ...
    patients_data.append({'id': i, 'pain': pain, 'infection': infection, 'priority': z3.IntVal(pain + infection), 'revenue': revenue})
...
if result == z3.sat:
    ...
    for j in range(num_hours):
        for i in range(num_patients):
            if model[appointment_slots[i][j]] == 1:
                ...
                print(f"\tRevenue: {patients_data[i]['revenue']}, Priority: {patients_data[i]['priority']}")
    ...
    for p in unscheduled:
        for q in scheduled:
            s.assert_and_track(p['priority'] <= q['priority'],\
                               f"Unscheduled patient {p['id']} priority {p['priority']} should not be greater than scheduled patient {q['id']} priority {q['priority']}") 

    print("\n===check if any unscheduled patient has higher priority than scheduled patients===")
    print(s.check())
